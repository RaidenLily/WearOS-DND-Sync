...
public class DndReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (NotificationManager.ACTION_INTERRUPTION_FILTER_CHANGED.equals(intent.getAction())) {
            ...
            taskQueue.execute(() -> sendToPhone(context, currentFilter));
        }
    }

    private void sendToPhone(Context context, int state) {
        ...
        for (BluetoothDevice device : pairedDevices) {
            ...
            try (BluetoothSocket socket = device.createInsecureRfcommSocketToServiceRecord(Constants.UUID_TO_PHONE)) {
                socket.connect();
